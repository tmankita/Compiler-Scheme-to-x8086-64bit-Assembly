...
let rec fill_index=
  ...
  let rec build_fvars_table=
    ...
    let rec labelInFVarTable =
      ...
      | []->""
      | (carName,index)::cdr-> if (String.equal carName fvar) then (concate_string_list (["fvar+";string_of_int index;"*WORD_SIZE"],"")) else  labelInFVarTable(cdr,fvar);;
...
try
  ...
  let code_fragment = String.concat "\n\n"
                        (List.map
                           ...
                           asts) in
  let final_code_fragment=concate_string_list ([code_fragment;],"") in
