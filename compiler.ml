...
= struct
  ...
  let normalize_scheme_symbol str =
    ...
    let make_spaced _p_ = 
      ...
      let make_enclosed_right  _p_ _r_ =
        let _enclosed_ = (PC.caten  _p_ _r_) in
        PC.pack _enclosed_ (fun (p,r) -> p);;
        
      let make_spaced_from_right _p_  = 
        let _st_space_ = PC.star _space_ in
        make_enclosed_right  _p_ _st_space_;;
...
let rec fill_index=
  ...
  let rec build_fvars_table=
    ...
    let rec labelInFVarTable =
      ...
      | []->""
      | (carName,index)::cdr-> if (String.equal carName fvar) then (concate_string_list (["fvar+";string_of_int index;"*WORD_BYTES"],"")) else  labelInFVarTable(cdr,fvar);;
...
module Code_Gen : CODE_GEN = struct
  let make_consts_tbl asts =  let tables_tuple_3= (List.map final_const_tbl asts) in 
                (List.flatten (List.map (fun (table)->(List.map (fun (a,b,c)->(a,(b,c))) table)) tables_tuple_3)) ;;
...
let rec find_in_const_table=
  ...
  let rec labelInFVarTable =
    ...
    match fvarTable with
    | []->""
    | (carName,index)::cdr-> if (String.equal carName fvar) then ("fvar_tbl+"^string_of_int index^"*WORD_BYTES") else  labelInFVarTable(cdr,fvar);;
